...
describe("E2E: pipeline", () => {
  it("has articles with title_tokens populated (kuromoji)", () => {
    const count = d1Count("SELECT COUNT(*) as cnt FROM articles WHERE title_tokens IS NOT NULL");
    expect(count).toBeGreaterThan(0);
  });

  it("has articles with embedded_at populated (bge-m3)", () => {
    const count = d1Count("SELECT COUNT(*) as cnt FROM articles WHERE embedded_at IS NOT NULL");
