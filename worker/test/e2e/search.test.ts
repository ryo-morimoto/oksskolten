...
describe("E2E: search", () => {
  ...
  it("semantic search finds related articles without exact keyword match", async () => {
    ...
    const res = await authFetch("/api/articles/search?q=serverless+computing");
    expect(res.status).toBe(200);
    const body = await res.json<{
  ...
  it("RRF merges FTS5 and Vectorize results", async () => {
    ...
    for (let i = 1; i < body.articles.length; i++) {
      expect(body.articles[i - 1].rank + 1e-10).toBeGreaterThanOrEqual(body.articles[i].rank);
    }
  });

  it("pagination is stable after RRF fusion", async () => {
    const page1 = await authFetch("/api/articles/search?q=Cloudflare&limit=3&offset=0");
    const page2 = await authFetch("/api/articles/search?q=Cloudflare&limit=3&offset=3");
