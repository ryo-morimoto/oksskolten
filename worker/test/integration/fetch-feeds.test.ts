...
import { startFeedWorkflows } from '../../src/pipeline/fetch-feeds'

describe('startFeedWorkflows', () => {
  beforeEach(async () => {
    await setupTestDb()
  })
  ...
  it('starts workflows only for enabled feeds with rss_url due for check', async () => {
    ...
    const started = await startFeedWorkflows(env as unknown as import('../../src/index').Env)
