...
describe("Article API", () => {
  ...
  describe("GET /api/articles", () => {
    ...
    it("sorts by quality_score when sort=score", async () => {
      const a = await seedArticle(feedId, { title: "Low Quality", url: "https://a.com/low" });
      const b = await seedArticle(feedId, { title: "High Quality", url: "https://a.com/high" });
      await env.DB.prepare("UPDATE articles SET quality_score = 0.2 WHERE id = ?")
        .bind(a.id as number)
        .run();
      await env.DB.prepare("UPDATE articles SET quality_score = 0.9 WHERE id = ?")
        .bind(b.id as number)
        .run();

      const res = await api("/articles?sort=score");
      const body = await res.json<{ articles: { title: string; quality_score: number }[] }>();
      expect(body.articles[0]?.title).toBe("High Quality");
      expect(body.articles[0]?.quality_score).toBeGreaterThan(body.articles[1]?.quality_score ?? 0);
    });

    it("filters by bookmarked", async () => {
      const a = await seedArticle(feedId);
      await env.DB.prepare("UPDATE articles SET bookmarked_at = datetime('now') WHERE id = ?")
        .bind(a.id as number)
        .run();
