...
describe('D1 migrations', () => {
  ...
  it('active_articles excludes purged articles', async () => {
    ...
    ).bind(feedId, 'Active', 'https://a.com/1').run()

    await env.DB.prepare(
      "INSERT INTO articles (feed_id, title, url, purged_at) VALUES (?, ?, ?, datetime('now'))",
    ).bind(feedId, 'Purged', 'https://a.com/2').run()
    ...
    const active = await env.DB.prepare(
      'SELECT COUNT(*) as count FROM active_articles',
    ).first<{ count: number }>()
  ...
  it('enforces unique article URL constraint', async () => {
    ...
    await env.DB.prepare(
      'INSERT INTO articles (feed_id, title, url) VALUES (?, ?, ?)',
    ).bind(feed!.id, 'a1', 'https://a.com/same').run()

    await expect(
      env.DB.prepare(
        'INSERT INTO articles (feed_id, title, url) VALUES (?, ?, ?)',
      ).bind(feed!.id, 'a2', 'https://a.com/same').run(),
    ).rejects.toThrow()
  })
})
