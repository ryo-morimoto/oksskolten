...
describe("FTS5 sync triggers", () => {
  ...
  async function ftsSearch(query: string) {
    return env.DB.prepare("SELECT rowid FROM articles_fts WHERE articles_fts MATCH ?")
      .bind(query)
      .all<{ rowid: number }>();
  }
  ...
  it("syncs FTS on INSERT with tokens", async () => {
    ...
    await env.DB.prepare(
      ...
      .bind(feed.id, "テスト記事", "https://example.com/1", "東京 天気", "東京 都 天気 予報")
      .run();
  ...
  it("syncs FTS on UPDATE of tokens", async () => {
    ...
    // Update tokens
    await env.DB.prepare(`UPDATE articles SET title_tokens = ?, full_text_tokens = ? WHERE url = ?`)
  ...
  it("removes from FTS on DELETE", async () => {
    ...
    await env.DB.prepare(`DELETE FROM articles WHERE url = ?`).bind("https://example.com/4").run();
  ...
  it("removes from FTS on purge (soft-delete)", async () => {
    ...
    await env.DB.prepare(`UPDATE articles SET purged_at = datetime('now') WHERE url = ?`)
  ...
  it("handles UPDATE replacing existing tokens", async () => {
    ...
    await env.DB.prepare(`UPDATE articles SET title_tokens = ? WHERE url = ?`)
