...
describe("D1 client helpers", () => {
  ...
  describe("bindNamedParams", () => {
    it("replaces @param with positional ?", () => {
      const result = bindNamedParams("SELECT * FROM feeds WHERE id = @id AND type = @type", {
        id: 1,
        type: "rss",
      });
    ...
    it("handles multiple occurrences of same param", () => {
      ...
      expect(result.sql).toBe("SELECT * FROM articles WHERE feed_id = ? OR category_id = ?");
      expect(result.args).toEqual([5, 5]);
    });

    it("throws on missing param", () => {
      expect(() => bindNamedParams("SELECT * FROM feeds WHERE id = @id", {})).toThrow(
        "Missing SQL parameter: id",
      );
    });
  ...
  describe("getNamed", () => {
    it("returns a single row", async () => {
      await env.DB.prepare("INSERT INTO categories (name) VALUES ('Tech')").run();
    ...
    it("returns null for no match", async () => {
      const row = await getNamed(env.DB, "SELECT * FROM categories WHERE name = @name", {
        name: "nonexistent",
      });
      expect(row).toBeNull();
    });
