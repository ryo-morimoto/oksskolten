...
feedRoutes.post("/feeds", async (c) => {
  ...
  // Check duplicate
  const existing = await c.env.DB.prepare("SELECT id FROM feeds WHERE url = ?")
    .bind(body.url)
    .first();
  if (existing) {
...
feedRoutes.patch("/feeds/:id", async (c) => {
  ...
  const existing = await c.env.DB.prepare("SELECT * FROM feeds WHERE id = ?").bind(id).first();
  ...
  const stmts = [
    c.env.DB.prepare(`UPDATE feeds SET ${sets.join(", ")} WHERE id = ?`).bind(...values),
  ];

  // Cascade category_id to articles
  if (body.category_id !== undefined) {
    stmts.push(
      c.env.DB.prepare("UPDATE articles SET category_id = ? WHERE feed_id = ?").bind(
        body.category_id,
        id,
      ),
    );
...
feedRoutes.delete("/feeds/:id", async (c) => {
  ...
  const articleIds = await c.env.DB.prepare("SELECT id FROM articles WHERE feed_id = ?")
    .bind(id)
    .all<{ id: number }>();
