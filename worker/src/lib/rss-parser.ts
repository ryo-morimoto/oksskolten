          ...
          .map((item) => {
            ...
            const excerpt =
              typeof rawExcerpt === "string"
                ? rawExcerpt
                : rawExcerpt && typeof rawExcerpt === "object" && "value" in rawExcerpt
      ...
      entries
        .map((entry: Record<string, unknown>) => {
          const link = Array.isArray(entry.link)
            ? (entry.link as Record<string, string>[]).find((l) => l["@_rel"] === "alternate")?.[
                "@_href"
          ...
          return {
            ...
            url: effectiveUrl,
            published_at: normalizeDate((entry.published || entry.updated) as string | undefined),
            ...(excerpt ? { excerpt } : {}),
          };
        })
        .filter((item: RssItem) => item.url),
