...
export interface QualityInput {
  markdown: string
  tokenCount?: number // kuromoji token count (Japanese word count)
}
...
function countHeadings(markdown: string): { count: number; hasH2: boolean; hasH3: boolean; hasH4: boolean } {
...
export function computeQualityScore(input: QualityInput): number {
  ...
  const headingDepthBonus = headings.hasH2 && headings.hasH3 && headings.hasH4
    ? 1.0
  ...
  return (
    0.30 * normalize(wordCount, 300, 5000) +
    0.20 * normalize(externalLinkDensity, 0, 10) +
    0.15 * normalize(headingDensity, 0, 8) +
    0.12 * normalize(images, 0, 8) +
    0.10 * normalize(codeBlocks, 0, 10) +
