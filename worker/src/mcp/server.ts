...
export function createMcpServer(env: Env, origin: string): McpServer {
  ...
  server.registerTool(
    "list_articles",
    {
      ...
      inputSchema: {
        ...
        sort: z
          .enum(["published_at", "score"])
          .optional()
          .describe("Sort order (default: published_at)"),
    ...
    async (params) => {
      ...
      if (params.feed_id != null) {
        conditions.push("a.feed_id = ?");
        binds.push(params.feed_id);
      }
  ...
  server.registerTool(
    ...
    async ({ type, limit }) => {
      const n = limit ?? 10;
      const col =
        type === "read" ? "read_at" : type === "bookmarked" ? "bookmarked_at" : "liked_at";
      const result = await db
        .prepare(
  ...
  server.registerTool(
    ...
    {
      description:
        "Infer user preferences from engagement data: top feeds, categories, and reading patterns",
      annotations: { readOnlyHint: true },
    },
    async () => {
  ...
  server.registerTool(
    ...
    {
      description:
        "Add a new RSS feed. Returns the created feed or an error if the URL already exists.",
      inputSchema: {
        ...
        rss_url: z
          ...
          .url()
          .optional()
          .describe("RSS/Atom feed URL if different from site URL"),
        category_id: z.number().optional().describe("Category ID to assign"),
      },
      annotations: { readOnlyHint: false },
    },
    async ({ url, name, rss_url, category_id }) => {
      const existing = await db.prepare("SELECT id FROM feeds WHERE url = ?").bind(url).first();
  ...
  server.registerTool(
    ...
    {
      description:
        "Disable or re-enable a feed. Disabled feeds stop fetching new articles but existing articles remain.",
      inputSchema: {
    ...
    async ({ id, disabled }) => {
      const sets = disabled ? "disabled = 1" : "disabled = 0, error_count = 0, last_error = NULL";
      const result = await db
  ...
  server.registerTool(
    ...
    async ({ id, limit }) => {
      ...
      const rowById = new Map(
        result.results.map((r: Record<string, unknown>) => [r.id as number, r]),
      );
  ...
  // ── Recommendation + MCP Apps UI ────────────────────────

  const articlesResourceUri = "ui://oksskolten/articles";
  ...
  registerAppTool(
    ...
    {
      description:
        "Get recommended articles ranked by structural quality, feed interest, and recency. Returns the best articles to read right now.",
      inputSchema: {
        feed_id: z.number().optional().describe("Filter to a specific feed"),
        ...
        after: z.string().optional().describe("ISO 8601 datetime or date (e.g. '2026-04-02') — only articles published after this"),
        before: z.string().optional().describe("ISO 8601 datetime or date — only articles published before this"),
        timezone: z.string().optional().describe("IANA timezone (e.g. 'Asia/Tokyo') — used to interpret after/before when no offset is provided"),
        limit: z.number().min(1).max(50).optional().describe("Number of results (default: 10)"),
        offset: z.number().min(0).optional().describe("Pagination offset"),
      },
      _meta: { ui: { resourceUri: articlesResourceUri } },
    },
    async (params) => {
  ...
  server.registerTool(
    ...
    {
      description:
        "Get per-feed interest scores and quality distribution. Shows which feeds produce high-quality content the user engages with.",
      annotations: { readOnlyHint: true },
    },
    async () => {
