...
opmlRoutes.post("/opml", async (c) => {
  ...
  const existingCategories = await c.env.DB.prepare("SELECT * FROM categories").all<{
    id: number;
    name: string;
  }>();
  ...
  for (const entry of parsed) {
    try {
      ...
      if (entry.categoryName) {
        ...
        if (existingCat) {
          ...
          const created = await c.env.DB.prepare(
            ...
            .bind(entry.categoryName, maxOrder!.next)
      ...
      await c.env.DB.prepare(
        ...
        .bind(entry.name, entry.url, entry.rssUrl, categoryId)
        .run();
      imported++;
    } catch (err) {
...
function parseOpml(xml: string): ParsedFeed[] {
  ...
  const outlines: OpmlOutline[] = Array.isArray(body.outline)
    ? body.outline
    : body.outline
      ? [body.outline]
      : [];
