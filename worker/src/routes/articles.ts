...
articleRoutes.get('/articles', async (c) => {
  ...
  if (feedId) { conditions.push('a.feed_id = ?'); binds.push(Number(feedId)) }
  if (categoryId) { conditions.push('a.category_id = ?'); binds.push(Number(categoryId)) }
  if (unread) { conditions.push('a.seen_at IS NULL') }
  if (bookmarked) { conditions.push('a.bookmarked_at IS NOT NULL') }
  if (liked) { conditions.push('a.liked_at IS NOT NULL') }
  ...
  const result = await c.env.DB.prepare(
    `SELECT a.id, a.feed_id, a.category_id, a.title, a.url, a.lang,
            a.excerpt, a.og_image, a.score, a.seen_at, a.read_at,
